...
class Attention(gnn.MessagePassing):
    ...
    def forward(self,
        ...
        if complete_edge_index is not None:
            ...
            qkv,ptr,mask=self.pos_encodding(x_struct, ptr, return_attn=return_attn)
            query=self.to_att(qkv)
            key = self.to_att(qkv)
            value = self.to_att(qkv)
            out, attn = self.self_attn(query,key,value, ptr, mask,return_attn=return_attn)#进行一次交叉注意力机制
    ...
    def pos_encodding(self, qkv, ptr, return_attn=False):
        qkv, mask = pad_batch(qkv, ptr, return_mask=True)
        # 进行位置编码
        batch_size, seq, model = qkv.shape
        pos_encodings = np.zeros((seq, model))
        for pos in range(seq):
            for i in range(model):
                if i % 2 == 0:
                    pos_encodings[pos, i] = np.sin(pos / (10000 ** (i / model)))
                else:
                    pos_encodings[pos, i] = np.cos(pos / (10000 ** ((i - 1) / model)))
        pos_encodings = np.expand_dims(pos_encodings, axis=0)  # Add batch dimension
        pos_encodings = np.tile(pos_encodings, (batch_size, 1, 1))
        pos_encodings=torch.FloatTensor(pos_encodings)
        qkv = qkv + pos_encodings.cuda()
        return qkv,ptr,mask

    def self_attn(self, q,k,v, ptr, mask,return_attn=False):
...
class StructureExtractor(nn.Module):
    ...
    def __init__(self, embed_dim, gnn_type="gcn", num_layers=2,
                 batch_norm=True, concat=True, khopgnn=False, **kwargs):
        super().__init__()
        self.num_layers = num_layers
        self.khopgnn = khopgnn
        self.concat = concat
        self.gnn1=gnn.GCNConv(embed_dim, embed_dim*4)
        self.gnn2 = gnn.GCNConv(embed_dim*4, embed_dim*2)
        self.gnn3 = gnn.GCNConv(embed_dim*2, embed_dim)
        self.gnn=gnn.GCNConv(embed_dim,embed_dim)
    ...
    def forward(self, x, edge_index, edge_attr=None,
        ...
        x = self.relu(self.gnn3(x, edge_index))
        sedu=self.relu(self.gnn(x,edge_index))
        x=x+sedu
